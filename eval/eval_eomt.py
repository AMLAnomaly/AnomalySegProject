...
def main():
    ...
    results_path = os.path.join('results', 'results_EoMT.txt')
    ...
    file.write("\n")
    file.write(f"----OOD EVALUATION EoMT (downsample={downsample_factor}) (MSP / MaxLogit / MaxEntropy / RbA)----\n")
    ...
    for dataset_id, nice_name in dataset_keys.items():
        ...
        current_gts = []
        current_scores = {"MSP": [], "MaxLogit": [], "MaxEntropy": [], "RbA": []}
        ...
        for path in current_dataset_paths:
            ...
            with torch.no_grad():
                ...
                pixel_logits_list = lit_model.revert_window_logits_semantic(crop_logits, origins, img_sizes)
                pixel_logits = pixel_logits_list[0].unsqueeze(0) 
                rba_score = compute_rba_score(pixel_logits)

            logits = pixel_logits.squeeze(0)  
            rba_anomaly = rba_score.squeeze(0)  # [H, W]
            del pixel_logits, rba_score
            ...
            # MaxEntropy
            entropy = -(probs * (probs + 1e-12).log()).sum(dim=0)
            anomaly_entropy = entropy

            del logits, probs, msp, maxlogit, entropy

            anomaly_msp = anomaly_msp.cpu().numpy()
            anomaly_maxlogit = anomaly_maxlogit.cpu().numpy()
            anomaly_entropy = anomaly_entropy.cpu().numpy()
            anomaly_rba = rba_anomaly.cpu().numpy()
            
            del rba_anomaly

            if downsample_factor > 1:
                anomaly_msp = block_reduce(anomaly_msp, (downsample_factor, downsample_factor), np.mean)
                anomaly_maxlogit = block_reduce(anomaly_maxlogit, (downsample_factor, downsample_factor), np.mean)
                anomaly_entropy = block_reduce(anomaly_entropy, (downsample_factor, downsample_factor), np.mean)
                anomaly_rba = block_reduce(anomaly_rba, (downsample_factor, downsample_factor), np.mean)
            ...
            current_scores["MSP"].append(anomaly_msp)
            current_scores["MaxLogit"].append(anomaly_maxlogit)
            current_scores["MaxEntropy"].append(anomaly_entropy)
            current_scores["RbA"].append(anomaly_rba)
