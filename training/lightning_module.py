...
class LightningModule(lightning.LightningModule):
    def __init__(
        ...
        outlier_loss_target: str = "nls",
        outlier_loss_func: str = "squared_hinge",
        inlier_upper_threshold: float = -0.0,
        outlier_lower_threshold: float = 5.0,
        score_norm: str = "tanh",
        ...
        if freeze_encoder:
            ...
            if num_unfrozen_encoder_blocks > 0:
                logging.info(f"Unfreezing last {num_unfrozen_encoder_blocks} encoder blocks")
                if hasattr(self.network.encoder.backbone, "blocks"):
                    total_blocks = len(self.network.encoder.backbone.blocks)
                    start_block = max(0, total_blocks - num_unfrozen_encoder_blocks)
                    for i in range(start_block, total_blocks):
                        for param in self.network.encoder.backbone.blocks[i].parameters():
                            param.requires_grad = True
                
                # Always unfreeze last norm if partial unfreezing is requested
